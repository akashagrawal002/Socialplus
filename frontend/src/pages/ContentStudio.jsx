...
export default function ContentStudio() {
  ...
  async function generate(withCompetitor = false) {
    ...
    try {
      const payload = { workspace_id: ws.id, ...form };
      if (withCompetitor) {
        payload.context = (form.context ? form.context + '\n' : '') +
          'Also analyse what competitors typically do with this topic and create differentiated content that stands out.';
      }
      ...
      <div className="card">
        ...
        <div className="input-group">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {CONTENT_TYPES.map((ct) => (
              <button
                key={ct}
                onClick={() => update('content_type', ct)}
                className={`text-xs px-3 py-1.5 rounded-full border transition-all
                  ${form.content_type === ct
                    ? 'bg-accent/20 border-accent/40 text-accent'
                    : 'bg-surface2 border-border text-text2 hover:border-accent/30'}`}
              >
                {ct}
              </button>
            ))}
        ...
        <div className="input-group">
          ...
          <textarea className="input" rows={3}
            placeholder="Key points, USP, brand voice, competitor insights, special instructions..."
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
          <div className="input-group">
            ...
            <select className="input" value={form.tone} onChange={(e) => update('tone', e.target.value)}>
              {['Professional', 'Conversational', 'Humorous', 'Inspirational', 'Educational', 'Bold & Provocative'].map((o) => (
          ...
          <div className="input-group">
            ...
            <select className="input" value={form.language} onChange={(e) => update('language', e.target.value)}>
              {['English', 'Hinglish (Hindi + English)', 'Hindi'].map((o) => (
                <option key={o}>{o}</option>
              ))}
            </select>
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button className="btn btn-secondary" onClick={() => generate(true)} disabled={loading}>
            🎯 Generate with Competitor Insight
          </button>
        </div>
